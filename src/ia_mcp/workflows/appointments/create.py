...
_T07_EVENTS = frozenset(
    {"collect_fields", "search_slots", "present_slots", "select_slot"}
)
...
def _create_arguments(
    slot: AppointmentSlot, patient: Mapping[str, object]
) -> dict[str, object]:
    request = AppointmentCreateRequest(
        slot_id=slot.slot_id,
        booking_token=slot.booking_token,
        patient=PatientRef.model_validate(dict(patient)),
    )
...
def _already_settled(execution: WorkflowExecution) -> bool:
    if execution.state in _TERMINAL:
        return True
    appointment_id = execution.data.get("appointment_id")
    return isinstance(appointment_id, str) and bool(appointment_id)
...
class CreateAppointmentDefinition:
    ...
    async def confirm_create(
        ...
        *,
        ...
        return await self._create_and_finish(
            engine,
            executor,
            tenant,
            workflow_id,
        ...
        *,
        ...
        result = await executor.execute(
            ...
            ToolCall(
                ...
                arguments=_create_arguments(live_slot, patient),
                idempotency_key=f"{workflow_id}:appointments.create",
            ),
        )
